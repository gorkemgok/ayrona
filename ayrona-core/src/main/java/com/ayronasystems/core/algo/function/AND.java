 ...
 */
@Fn(name = "AND", inputCount = 2, paramCount = 0, sameSizeInput = false)
public class AND extends AbstractNonPeriodicFunction{

    public FIOExchange calculate (FIOExchange inputs, double... params) {
        double[] output = new double[getOutputCount (inputs, params)];
        double[] in1 = inputs.getDataWithShortestSize (0);
        double[] in2 = inputs.getDataWithShortestSize (1);
